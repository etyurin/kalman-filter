...
(defn run-battery-test [steps real-voltage]
  (let [A (mat/matrix [[1]])
   ...
   (let [measure-battery (fn [n]
         ...
         estimated-battery-vs (:state (loop [curr-results (voltage-update-fn (mat/matrix [[3]])
                                                                             (mat/matrix [[1]])
                                                                             (mat/matrix [[0]])
                                                                             (mat/matrix [[(measure-battery 1)]]))
                                            idx 0
                                            state-history []
                                            covar-history []]
                                       (if (>= idx steps)
                                         {:state state-history :covar-matrix covar-history}
                                         (recur (voltage-update-fn (:state curr-results) (:covar-matrix curr-results) (mat/matrix [[0]]) (mat/matrix [[(measure-battery 1)]]))
                                                (inc idx)
                                                ...
                                                (conj covar-history (mat/mget (:covar-matrix curr-results) 0 0))))))
